...
function renderCartItems() {
    ...
    if (window.cart.length === 0) {
        ...
        window.cart.forEach((item, index) => {
            ...
            html += `
                <div class="cart-line-item flex items-center justify-between py-3 border-b border-gray-100 gap-3">
                    <img src="${imageUrlToDisplay}" alt="${item.name}" class="w-16 h-16 flex-shrink-0 object-cover rounded-md" onerror="this.onerror=null;this.src='${window.placeholderImageUrl || '/static/img/sin_imagen.jpg'}';">
                    <div class="cart-line-details flex-1 min-w-0">
                    ...
                    <div class="cart-line-actions flex items-center gap-1 flex-shrink-0">
                        <button type="button" onclick="updateItemQuantity('${item.variantId}', -1)" class="text-red-500 p-2 rounded-full hover:bg-red-100 transition-colors" aria-label="Reducir cantidad">
                            <i class="fas fa-minus-circle text-lg"></i>
                        </button>
                        <span class="font-semibold text-gray-800 min-w-[1.5rem] text-center">${itemQuantity}</span>
                        <button type="button" onclick="updateItemQuantity('${item.variantId}', 1)" class="text-green-600 p-2 rounded-full hover:bg-green-100 transition-colors" aria-label="Aumentar cantidad">
                            <i class="fas fa-plus-circle text-lg"></i>
                        </button>
                        <button type="button" onclick="updateItemQuantity('${item.variantId}', 0)" class="text-gray-500 hover:text-red-700 p-2" aria-label="Eliminar">
...
function getProductIdFromButton(button) {
    ...
    return button.dataset.productId || button.getAttribute("data-product-id");
}
...
function applyFavoriteState(productId, isFavorite) {
    ...
    buttons.forEach((button) => {
        ...
        icon.classList.remove("fa-regular", "fa-solid", "fa-heart", "text-gray-500", "text-red-500", "animate-bounce");
...
function updateFavoritesEmptyState() {
    ...
    const remaining = container.querySelectorAll(".product-card").length;
...
function removeFavoriteCard(productId) {
    ...
    const card = container.querySelector(`.product-card[data-product-id="${productId}"]`);
    if (!card) return;
...
async function toggleFavorito(button, productoId) {
    ...
    try {
        const response = await fetch("/toggle-favorito/", {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                "X-CSRFToken": getCookie("csrftoken"),
        ...
        localStorage.setItem(`favorito-${productoId}`, data.is_favorito ? "true" : "false");

        if (!data.is_favorito) {
            removeFavoriteCard(productoId);
        }

        if (typeof showFavoriteMessage === "function" && data.mensaje) {
            showFavoriteMessage(data.mensaje);
        }
    } catch (error) {
        console.error("Error al actualizar favoritos:", error);
        applyFavoriteState(productoId, wasFavorite);
        localStorage.setItem(`favorito-${productoId}`, wasFavorite ? "true" : "false");
    }
}

function syncFavoritosFromServerMarkup() {
    document.querySelectorAll(".btn-favorito").forEach((btn) => {
        const productId = getProductIdFromButton(btn);
        if (!productId) return;
        localStorage.setItem(`favorito-${productId}`, btn.classList.contains("active") ? "true" : "false");
    });
}
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (bottomNavCategoriesButton && mobileMenu) {
    ...
    document.addEventListener("click", (event) => {
        ...
        const productoId = getProductIdFromButton(btn);
